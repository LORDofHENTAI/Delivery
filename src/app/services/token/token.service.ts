...
import { LoginResponse } from 'src/app/components/login-menagment/models/login-responce.model';
@Injectable({
  providedIn: 'root'
})
export class TokenService {
  ...
  getTitle() {
    try {
      if (this.cookieService.check(this.cookieName)) {
        let fullData = this.cookieService.get(this.cookieName)
        let titleFromCookie = JSON.parse(fullData)
        if (titleFromCookie) {
          return titleFromCookie.title
        } else return false
      }
      else return false
    } catch {
